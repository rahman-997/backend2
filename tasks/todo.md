...
- [x] Session 3: Serializable booking transaction with bounded retry; cancelled rows can rebook.
...
## 1.0 production milestone

- [x] API/worker structured logs, metrics, dependency readiness and graceful shutdown.
- [x] Production PostgreSQL indexes and adaptive outbox polling.
- [x] Redis runtime isolation between API and worker.
- [x] Migration startup backoff for overlapping Render deploys.
- [x] Installable PWA manifest, icon and install UX.
- [x] Offline app shell with API traffic explicitly excluded from service-worker caching.
- [x] Immutable caching for hashed assets and revalidation for HTML/service-worker metadata.
- [x] PWA artifact verification added to the frontend release gate.
- [x] Eventify package promoted to 1.0.0 with changelog.

## External integration still optional
